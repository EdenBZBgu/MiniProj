...
# Base Classifier
class WCharacteristicClassifier(ABC):
...
class WCharacteristicLogisticRegressionClassifier(WCharacteristicClassifier):
    def initialize_model(self):
        self.model = LogisticRegression(max_iter = 500)
        self.model_name = "Logistic Regression"


class WCharacteristicRidgeClassifierModel(WCharacteristicClassifier):
    def initialize_model(self):
        self.model = RidgeClassifier()
        self.model_name = "Ridge Classifier"


class WCharacteristicSVMClassifier(WCharacteristicClassifier):
    def initialize_model(self):
        self.model = SVC(kernel = "linear")
        self.model_name = "SVM"


class WCharacteristicKNNClassifier(WCharacteristicClassifier):
    def __init__(self, torah : Torah, pickle_file = "torah.pkl", test_size = 0.2, random_state = 42, isBook = True, n_neighbors = 10):
        super(WCharacteristicKNNClassifier, self).__init__(torah, pickle_file, test_size, random_state, isBook)
...
class WCharacteristicMLPClassifierModel(WCharacteristicClassifier):
    def initialize_model(self):
        self.model = MLPClassifier(max_iter = 1000)
        self.model_name = "MLP Classifier"


class WCharacteristicSGDClassifierModel(WCharacteristicClassifier):
    def initialize_model(self):
