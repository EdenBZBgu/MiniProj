...
class RootClassifier(ABC):
    ...
    def test(self):
        ...
        if self.isBook:
            report = classification_report(self.y_test, y_pred, target_names=[f"Book {book.book_name}" for book in self.torah.books])
        else:
            report = classification_report(self.y_test, y_pred, target_names=[f"Teuda {teuda.teuda_name}" for teuda in self.torah.teudot])
